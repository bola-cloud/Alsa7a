...
class RequestStatusUpdated extends Notification
{
    ...
    public function toArray(object $notifiable): array
    {
        ...
        $bodies = ['en' => "Your request status is now: $status", 'ar' => "حالة طلبك الآن: $status"];

        switch ($status) {
            case 'accepted':
                $titles = ['en' => 'Request Accepted', 'ar' => 'تم قبول الطلب'];
                $bodies = ['en' => "Your request for $serviceTitle has been accepted!", 'ar' => "تم قبول طلبك لخدمة $serviceTitle!"];
                break;
            case 'rejected':
                $titles = ['en' => 'Request Declined', 'ar' => 'تم رفض الطلب'];
                $bodies = ['en' => "Your request for $serviceTitle has been declined.", 'ar' => "تم رفض طلبك لخدمة $serviceTitle."];
                break;
            case 'completed':
                $titles = ['en' => 'Service Completed', 'ar' => 'تم إكمال الخدمة'];
                $bodies = ['en' => "Your request for $serviceTitle is marked as completed.", 'ar' => "تم تحديد طلبك لخدمة $serviceTitle كمكتمل."];
                break;
            case 'canceled':
                $titles = ['en' => 'Request Canceled', 'ar' => 'تم إلغاء الطلب'];
                $bodies = ['en' => "Your request for $serviceTitle has been canceled.", 'ar' => "تم إلغاء طلبك لخدمة $serviceTitle."];
                break;
        }
