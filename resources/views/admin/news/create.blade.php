...
@section('content')
    <div class="content-header row mb-4">
        <div class="content-header-left col-md-12 col-12 mb-2">
            ...
            <h3 class="content-header-title mb-0">{{ __('admin.news.create') }}</h3>
        </div>
    </div>
                    ...
                    <form action="{{ route('admin.news.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="col-md-8">
                                @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                    <div class="form-group">
                                        <label>{{ __('admin.categories.name') }} ({{ $properties['native'] }}) <span
                                                class="text-danger">*</span></label>
                                        <input type="text" name="title[{{ $localeCode }}]" class="form-control" required
                                            value="{{ old('title.' . $localeCode) }}"
                                            placeholder="{{ __('admin.categories.name') }} ({{ $properties['native'] }})">
                                    </div>
                                    <div class="form-group">
                                        <label>{{ __('admin.categories.description') }} ({{ $properties['native'] }})</label>
                                        <textarea name="content[{{ $localeCode }}]" class="form-control" rows="10"
                                            placeholder="{{ __('admin.categories.description') }} ({{ $properties['native'] }})">{{ old('content.' . $localeCode) }}</textarea>
                                    </div>
                                @endforeach
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="form-group">
                                    <label>{{ __('admin.news.image') }}</label>
                                    <div class="custom-file">
                                        <input type="file" class="custom-file-input" name="image" id="newsImage">
                                        <label class="custom-file-label"
                                            for="newsImage">{{ __('admin.news.choose_file') }}</label>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="custom-file mb-1">
                                        <input type="file" class="custom-file-input" id="gallery-upload" multiple
                                            accept="image/*">
                                        <label class="custom-file-label"
                                            for="gallery-upload">{{ __('admin.news.choose_images') }}</label>
                                    </div>
                                    <input type="file" name="images[]" id="real-gallery-input" multiple
                                        style="display: none;">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="custom-file mb-2">
                                        ...
                                        <label class="custom-file-label"
                                            for="newsVideo">{{ __('admin.news.choose_video') }}</label>
                                    </div>
                                    <input type="url" name="video_url" class="form-control"
                                        placeholder="{{ __('admin.news.video_url') }}">
                                </div>
                            </div>
                        </div>
