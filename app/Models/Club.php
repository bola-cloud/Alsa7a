...
class Club extends Model
{
    ...
    {
        if (! $value) { $this->attributes['logo_url'] = $value; return; }
        if (! preg_match('#^https?://#i', $value)) { $this->attributes['logo_url'] = ltrim($value, '/'); return; }
    ...
    {
        if (! $value) { $this->attributes['banner_url'] = $value; return; }
        if (! preg_match('#^https?://#i', $value)) { $this->attributes['banner_url'] = ltrim($value, '/'); return; }
        $appHost = parse_url(config('app.url') ?? url('/'), PHP_URL_HOST);
        $givenHost = parse_url($value, PHP_URL_HOST);
        if ($appHost && $givenHost && strtolower($appHost) === strtolower($givenHost)) {
